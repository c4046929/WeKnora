...
// GetModelUsage returns model-level usage generated by evaluation runs.
// @Summary      获取评测模型用量
// @Description  按模型聚合当前租户的评测调用量、Token、缓存、耗时和成本
// @Tags         评估
// @Produce      json
...
func (e *EvaluationHandler) GetModelUsage(c *gin.Context) {
	ctx := c.Request.Context()
	stats, err := e.evaluationService.ModelUsage(ctx)
	if err != nil {
		logger.ErrorWithFields(ctx, err, nil)
		c.Error(errors.NewInternalServerError(err.Error()))
		return
